...
class QsoBot:
    bot_messages = {
        ...
        'MIDI': {
            'CALL'       : [r'(call)(\s+(hr|here|is))*\s+(?P<CALL>\S+)'    , 'dr {OP} , my call is {OWN_CALL} {OWN_CALL} {OWN_CALL}'           ],
            'CALL_SIMPLE': [r'(call)'                                      , 'dr om ,  my call is {OWN_CALL} {OWN_CALL} {OWN_CALL}'            ],
            'NAME'       : [r'(op|name|am)(\s+(hr|here|is))*\s+(?P<OP>\S+)', 'dr {OP} , my name is {OWN_NAME} {OWN_NAME} {OWN_NAME}'           ],
            'NAME_SIMPLE': [r'(op|name|am)'                                , 'dr om , ur name? = my name is {OWN_NAME} {OWN_NAME} {OWN_NAME}'  ],
            'QTH'        : [r'(loc|qth)(\s+(hr|here|is))*\s+(?P<QTH>\S+)'  , 'dr {OP} fm {QTH} , my qth is {OWN_QTH} {OWN_QTH} {OWN_QTH}'    ],
            'QTH_SIMPLE' : [r'(loc|qth)'                                   , 'my qth is {OWN_QTH} {OWN_QTH} {OWN_QTH} = ur qth?'             ],
            'WX'         : [r'(wx)(\s+(hr|here|is))*\s+(?P<WX>\S+)'        , 'wx hr is {OWN_WX} {OWN_WX} {OWN_WX}'                         ],
            'WX_SIMPLE'  : [r'(wx)'                                        , 'wx hr is {OWN_WX} {OWN_WX} {OWN_WX}'                         ],
            'TMP'        : [r'(tmp)(\s+(hr|here|is))*\s+(?P<TMP>\S+(c|f)?)', 'temp hr is {OWN_TEMP} {OWN_TEMP} {OWN_TEMP}'                     ],
            'TMP_SIMPLE' : [r'(temp)'                                      , 'temp hr is {OWN_TEMP} {OWN_TEMP}{OWN_TEMP}'                     ],
            'RIG'        : [r'(rig)(\s+(hr|here|is))*\s+(?P<RIG>\S+)'      , 'my rig is {OWN_RIG} {OWN_RIG} {OWN_RIG}'                       ],
            'RIG_SIMPLE' : [r'(rig)'                                       , 'my rig is {OWN_RIG} {OWN_RIG} {OWN_RIG}'                       ],
            'ANT'        : [r'(ant)(\s+(hr|here|is))*\s+(?P<ANT>\S+)'      , 'my ant is {OWN_ANT} {OWN_ANT} {OWN_ANT}'                       ],
            'ANT_SIMPLE' : [r'(ant)'                                       , 'my ant is {OWN_ANT} {OWN_ANT} {OWN_ANT}'                       ],
            'RST'        : [r'(rst)(\s+(is))*\s+(?P<OWN_RST>\S+)'          , 'ur rst is {UR_RST} {UR_RST} {UR_RST}'                        ],
            'RST_SIMPLE' : [r'(rst)'                                       , 'ur rst is {UR_RST} {UR_RST} {UR_RST} = dr {OP}, hw cpy?'     ],
